  ...
  <Container>
    ...
    <div class="mt-8 flex justify-center">
      <div class="relative">
        <div id="pricing-perion" role="radiogroup" class="grid grid-cols-2">
          {#each ['monthly', 'annually'] as period}
            ...
            <div
              ...
              class={[
                'cursor-pointer border border-green-300 px-[calc(theme(spacing.3)-1px)] py-[calc(theme(spacing.2)-1px)] text-center text-sm text-green-700 capitalize transition-colors hover:border-green-400 focus:outline-2 focus:outline-offset-2',
        ...
        >
          {#each ['monthly', 'annually'] as period}
            <div class="py-2 text-center text-sm font-semibold text-white capitalize {period === 'annually' ? '-ml-px' : ''}">
              {(m as any)[period]()}
            </div>
          {/each}
        </div>
