...
import { AnimatePresence, motion, type MotionProps, type Variant, type Variants } from 'framer-motion'
        ...
        <div className="px-4 py-6">
          <div className="space-y-6">
            {[
              ...
              <div key={field.label}>
                <div className="text-sm text-green-500">{field.label}</div>
                <div className="mt-2 border-b border-green-200 pb-2 text-sm text-green-900">{field.value}</div>
              </div>
            ))}
          </div>
        ...
        <div className="divide-y divide-green-100">
          {[
            ...
            <div key={stock.name} className="flex items-center gap-4 px-4 py-3">
              ...
              <div className="flex-auto text-sm text-green-900">{stock.name}</div>
              <div className="flex-none text-right">
                <div className="text-sm font-medium text-green-900">{stock.price}</div>
                <div className={clsx('text-xs leading-5', stock.change.startsWith('+') ? 'text-cyan-500' : 'text-green-500')}>
                  {stock.change}
                </div>
              </div>
            </div>
        ...
        <div className="px-4 py-6">
          <div className="space-y-4">
            {[
              ...
              <div key={item.label} className="flex justify-between border-b border-green-100 pb-4">
                <div className="text-sm text-green-500">{item.label}</div>
                <div className="text-sm font-semibold text-green-900">{item.value}</div>
              </div>
            ))}
    ...
    >
      <TabList className="relative z-10 order-last col-span-6 space-y-6">
        {features.map((feature, featureIndex) => (
          <div key={feature.name} className="relative rounded-2xl transition-colors hover:bg-green-800/30">
            {featureIndex === selectedIndex && (
              <motion.div layoutId="activeBackground" className="absolute inset-0 bg-green-800" initial={{ borderRadius: 16 }} />
            )}
            <div className="relative z-10 p-8">
              ...
              <p className="mt-2 text-sm text-green-400">{feature.description}</p>
            </div>
          </div>
        ))}
      </TabList>
    ...
    <>
      ...
      >
        {features.map((feature, featureIndex) => (
          ...
          >
            <div className="relative transform overflow-hidden rounded-2xl bg-green-800 px-5 py-6">
              <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2">
              ...
              <div className="absolute inset-x-0 bottom-0 bg-green-800/95 p-6 backdrop-blur sm:p-10">
                <feature.icon className="h-8 w-8" />
      ...
      <div className="mt-6 flex justify-center gap-3">
        {features.map((_, featureIndex) => (
          <button
            type="button"
            key={featureIndex}
            className={clsx('relative h-0.5 w-4 rounded-full', featureIndex === activeIndex ? 'bg-green-300' : 'bg-green-500')}
...
export function PrimaryFeatures() {
  return (
    <section id="features" aria-label="Features for investing all your money" className="bg-green-900 py-20 sm:py-32">
      <Container>
        <div className="mx-auto max-w-2xl lg:mx-0 lg:max-w-3xl">
          ...
          <p className="mt-2 text-lg text-green-400">
            Pocket was built for investors like you who play by their own rules and aren’t going to let SEC regulations get in the
            way of their dreams. If other investing tools are afraid to build it, Pocket has it.
          </p>
        </div>
      </Container>
