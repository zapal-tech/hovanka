  ...
  return (
    <div className="flex">
      {[...Array(5).keys()].map((index) => (
        <StarIcon key={index} className={clsx('h-5 w-5', rating > index ? 'fill-cyan-500' : 'fill-green-300')} />
      ))}
    </div>
  )
}
...
function Review({
  ...
  return (
    <figure
      className={clsx('animate-fade-in rounded-3xl bg-white p-6 opacity-0 shadow-md shadow-green-900/5', className)}
      style={{ animationDelay }}
      {...props}
    >
      <blockquote className="text-green-900">
        <StarRating rating={rating} />
        <p className="mt-4 text-lg leading-6 font-semibold before:content-['“'] after:content-['”']">{title}</p>
        <p className="mt-3 text-base leading-7">{body}</p>
      </blockquote>
      <figcaption className="mt-3 text-sm text-green-600 before:content-['–_']">{author}</figcaption>
    </figure>
  )
}
...
function ReviewGrid() {
  ...
  return (
    ...
    >
      ...
      <div className="pointer-events-none absolute inset-x-0 top-0 h-32 bg-gradient-to-b from-green-50" />
      <div className="pointer-events-none absolute inset-x-0 bottom-0 h-32 bg-gradient-to-t from-green-50" />
    </div>
  )
}
...
export function Reviews() {
  return (
    <section id="reviews" aria-labelledby="reviews-title" className="pt-20 pb-16 sm:pt-32 sm:pb-24">
      <Container>
        <h2 id="reviews-title" className="text-3xl font-medium tracking-tight text-green-900 sm:text-center">
          Everyone is changing their life with Pocket.
        </h2>
        <p className="mt-2 text-lg text-green-600 sm:text-center">
