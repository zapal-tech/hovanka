...
<section
  class={[
    ...
    featured ? 'bg-green-900 order-first lg:order-none' : 'bg-white',
  ]}
>
  ...
  <p class={['relative mt-5 gap-2 flex text-3xl tracking-tight', featured ? 'text-white' : 'text-green-900']}>
    ...
    {:else}
      <span
        ...
        class={[
          ...
          activePeriod !== 'monthly' && 'absolute top-0 left-0 pointer-events-none -translate-x-6 opacity-0 select-none',
        ]}
      >
      ...
      <span
        ...
        class={[
          'transition duration-300',
          activePeriod !== 'annually' && 'absolute top-0 left-0 pointer-events-none translate-x-6 opacity-0 select-none',
        ]}
      >
