...
<ul class={['flex gap-5 lg:gap-3', $$restProps.class]}>
  {#each languages as lang}
    <li>
      <a
        class={[
          'uppercase lg:py-[calc(--spacing(2)-1px)] lg:px-[calc(--spacing(3)-1px)] lg:rounded-lg lg:hover:bg-green-100 transition-colors',
          lang === runtime.languageTag() ? 'text-green-900 hover:text-green-700' : 'text-green-400 hover:text-green-700',
        ]}
        href={createI18n(runtime, { prefixDefaultLanguage: 'always' }).route($page.url.pathname)}
        hreflang={lang}
