...
const plans = [
  {
    ...
    logomarkClassName: 'fill-green-300',
  },
  {
    name: 'Investor',
    featured: false,
...
function Plan({
  ...
  return (
    ...
    >
      <h3 className={clsx('flex items-center text-sm font-semibold', featured ? 'text-white' : 'text-green-900')}>
        <Logomark className={clsx('h-6 w-6 flex-none', logomarkClassName)} />
        <span className="ml-4">{name}</span>
      </h3>
      <p className={clsx('relative mt-5 flex text-3xl tracking-tight', featured ? 'text-white' : 'text-green-900')}>
      ...
      <p className={clsx('mt-3 text-sm', featured ? 'text-green-300' : 'text-green-700')}>{description}</p>
      <div className="order-last mt-6">
        <ul
          role="list"
          className={clsx(
            '-my-2 divide-y text-sm',
            featured ? 'divide-green-800 text-green-300' : 'divide-green-200 text-green-700',
          )}
        >
...
export function Pricing() {
  ...
  return (
    <section id="pricing" aria-labelledby="pricing-title" className="border-t border-green-200 bg-green-100 py-20 sm:py-32">
      <Container>
        <div className="mx-auto max-w-2xl text-center">
          <h2 id="pricing-title" className="text-3xl font-medium tracking-tight text-green-900">
          ...
          <div className="relative">
            <RadioGroup value={activePeriod} onChange={setActivePeriod} className="grid grid-cols-2">
              {['Monthly', 'Annually'].map((period) => (
                <Radio
                  ...
                  value={period}
                  className={clsx(
                    'cursor-pointer border border-green-300 px-[calc(theme(spacing.3)-1px)] py-[calc(theme(spacing.2)-1px)] text-sm text-green-700 outline-2 outline-offset-2 transition-colors hover:border-green-400',
            ...
            >
              {['Monthly', 'Annually'].map((period) => (
                <div
                  key={period}
                  className={clsx('py-2 text-center text-sm font-semibold text-white', period === 'Annually' && '-ml-px')}
                >
                  {period}
                </div>
              ))}
