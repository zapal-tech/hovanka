...
<header>
  <nav>
    <Container class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 relative z-50 flex justify-between py-8">
      ...
      <div class="flex items-center gap-6">
        <div class="lg:hidden">
          ...
          {#if isOpen}
            ...
            <div
              use:popover.panel
              class="absolute inset-x-0 top-0 z-0 origin-top rounded-b-2xl bg-green-50 px-6 pb-6 pt-32 shadow-2xl shadow-green-900/20"
            >
              <div class="space-y-4">
              ...
              <div class="mt-8 flex flex-col gap-4">
                <Button href={`https://${PUBLIC_WEB_APP_DOMAIN}/sign-in`} variant="outline">{m.sign_in()}</Button>
                <Button href={`https://${PUBLIC_WEB_APP_DOMAIN}`}>{m.try_it_out()}</Button>
              </div>
            </div>
          {/if}
        </div>

        <LanguageSwitcher class="hidden lg:flex mr-4" />

        <Button href={`https://${PUBLIC_WEB_APP_DOMAIN}/sign-in`} target="_blank" variant="outline" class="hidden! lg:block!"
          >{m.sign_in()}</Button
        >
        <Button href={`https://${PUBLIC_WEB_APP_DOMAIN}`} target="_blank" class="hidden! lg:block!">{m.try_it_out()}</Button>
      </div>
    </Container>
  </nav>
</header>
